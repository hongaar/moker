...
const argument_js_1 = require("./argument.cjs");

const option_js_1 = require("./option.cjs");

const prompter_js_1 = require("./prompter.cjs");

function isArgument(obj) {
  return obj instanceof argument_js_1.Argument;
}

function isOption(obj) {
  return obj instanceof option_js_1.Option;
}
...
class Command {
  ...
  argument(name, options) {
    this.add(new argument_js_1.Argument(name, options));
    return this;
  }
  /**
  ...
  option(name, options) {
    this.add(new option_js_1.Option(name, options));
    return this;
  }
  /**
  ...
  toYargsCommand() {
    ...
    const args = this.getArguments().map(arg => arg.toCommand()).join(' ');

    if (args !== '') {
      return Array.isArray(this.command) ? [`${this.command[0]} ${args}`, ...this.command.slice(1)] : `${this.command} ${args}`;
    }
  ...
  getBuilder(commandRunner) {
    return yargs => {
      ...
      yargs = [...this.getArguments(), ...this.getOptions()].reduce((yargs, arg) => arg.toYargs(yargs), yargs); // Call toYargs on each subcommand to add it to the command.
  ...
  getHandler(commandRunner) {
    return async argv => {
      const prompterInstance = (0, prompter_js_1.prompter)([...this.getArguments(), ...this.getOptions()], argv);
      ...
      promise = promise.then(_a => {
        var {
          ...
          __promise
            ...
            args = __rest(_a, ["_", "$0", "__promise"]); // @todo coerce all types and remove coerce option from baseArg
