...
const prompt = enquirer_1.default.prompt;
/**
 * Creates a new prompter instance
 */

function prompter(baseArgs, args) {
...
class Prompter {
  ...
  async prompt() {
    const questions = this.getQuestions(this.args); // Short circuit if there are no questions to ask.
  ...
  getQuestions(args) {
    ...
    return this.baseArgs.reduce((questions, arg) => {
      ...
      const isDefault = presentInArgs && typeof defaultValue !== 'undefined' && defaultValue == args[name]; // We're going to assume that if an argument/option still has its default
      ...
      if (isPromptable && (!presentInArgs || isDefault)) {
        ...
        switch (true) {
          case typeof arg.getChoices() !== 'undefined' && (arg.getType() === 'array' || Array.isArray(defaultValue)):
...
exports.Prompter = Prompter;
//# sourceMappingURL=prompter.cjs.map
